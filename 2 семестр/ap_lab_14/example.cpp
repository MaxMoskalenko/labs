...
struct subject
{
    ...
    //назва дисципліни
    //викладач, що читає предмет
    //кількість студентів, що вивчають дану дисципліну
    stud student[10];
};
int i, j;
FILE *fp;
...
void input()
{
    ...
    else
    {
        ...
        do
        {
            ...
            do
            {
                //масив студентів, що вивчають предмет
                //лічильники викладачів і студентів //покажчик на файл
...
void list_prepod()
{
    ...
    else
    {
        ...
        while (!feof(fp))                        //доки не кінець файлу
        {
            ...
            cout << " " << setw(10) << predmet.name << setw(12) << predmet.prepod.fio << setw(14) << predmet.prepod.posada << predmet.prepod.salary << endl;
            fread(&predmet, sizeof(subject), 1, fp); //читання даних з 1 предмету
        }
    ...
    else
    {
        ...
        while (!feof(fp))
        {
            ...
            for (j = 0; j < predmet.stnum; j++)
            {
                cout.setf(ios::left);
                cout << " " <<
                    //перегляд студентів по даній
                    // дисципліні
                    //виведення даних по студентах
                    setw(12) << predmet.student[j].fio << setw(10) << predmet.student[j].bal << predmet.student[j].stip << endl;
            }
            fread(&predmet, sizeof(subject), 1, fp); //читання даних з 1 предмету
        }
        fclose(fp); //закриття файлу
...
void read_from_file()
{
    ...
    else
    {
        ...
        while (!feof(fp))
        {
            ...
            cout.setf(ios::left);
            cout << setw(12) << predmet.name << setw(12) << predmet.prepod.fio << setw(13) << predmet.prepod.posada /*<<setw(10)*/ << predmet.prepod.salary << endl;
            cout << "____Students" << endl;
            cout << " fio--------ball-----scholarship" << endl;
            for (j = 0; j < predmet.stnum; j++) //студенти
            {
                cout.setf(ios::left);
                cout << " " << setw(12) << predmet.student[j].fio << setw(10) << predmet.student[j].bal << predmet.student[j].stip << endl;
            }
            fread(&predmet, sizeof(subject), 1, fp); //читання даних з 1 предмету
        }
        fclose(fp); //закриття файлу
    ...
    else
    {
        ...
        while (!feof(fp))
        {
            ...
            for (j = 0; j < predmet[i].stnum; j++)   //перегляд студентів
                sr[i] += predmet[i].student[j].bal;  //сума балів студентів по //дисципліні
            sr[i] = float(sr[i]) / predmet[i].stnum; //середній бал по даному //предмету

            i++;                                        //перехід до наступного предмету
        ...
        for (i = 1; i < kol_prep; i++)
            if (max < sr[i])
            //максимальний бал
            //номер дисципліни(викладача) у масиві //дисциплін
            //перегляд дисциплін
            {
                max = sr[i]; //визначити максимальний бал й номер дисципліни
                nomer_predmet = i;
            }
        cout << "max ball=" << max << " in subject " << predmet[nomer_predmet].name << " teacher " << predmet[nomer_predmet].prepod.fio << endl;
        fclose(fp); //закриття файлу
    }
    getch(); //чекати натискання клавіші
}
...
void stipendia()
{
    ...
    stud s, tmp[10], stud_tmp[10]; //проміжні масиви студентів //копіювання даних по студентам у тимчасовий масив
    // без дисциплін та даних по викладачам
    ...
    else
    {
        ...
        for (i = 0; i < k; i++) //перегляд всього масиву студентів
        {
            ...
            if (f == 0)    //студент іще не зустрічався в масиві
            {
                ...
                //середній бал студента по всім предметам

                stud_tmp[l].bal = float(stud_tmp[l].bal) / kol[l];
                l++; //перехід до наступного студента
            }
        }
        int all_stud = l; //загальна кількість студентів //сортування студентів за алфавітом
        ...
        j = 0; //їх кількість for
        (i = 0; i < all_stud; i++) if ((stud_tmp[i].bal >= 0) && (stud_tmp[i].bal < 3))
        {
            sum += stud_tmp[i].stip;
            stud_tmp[i].stip = 0;
            j++;
...
for (i = 0; i < all_stud; i++)
{
    ...
    else cout << "has no scholarship" << endl;
    //стипендія не нарахована
    //чекати натискання клавіші
}
//---------------------стипендіальний фонд----------------------
void fond()
{
    ...
    subject predmet;
    stud tmp[10];
    //фонд викладачів //стипендіальний фонд //предмет
    //тимчасовий масив студентів
    if (!(fp = fopen("fp.dat", "rb")))
        cerr << "File do not exist!";
    else
    {
        ...
        while (!feof(fp))
        {
            ...
            //по даній дисципліні //копіювання масиву студентів
            tmp[k] = predmet.student[j];
            k++;
